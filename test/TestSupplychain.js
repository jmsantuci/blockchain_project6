...
contract('SupplyChain', function(accounts) {
    ...
    it("Testing smart contract function sellItem() that allows a farmer to sell coffee", async() => {
        ...
        // Retrieve the just now saved item from blockchain by calling function fetchIt em()
        const resultBufferTwo = await supplyChain.fetchItemBufferTwo.call(upc);
   ...
   it("Testing smart contract function shipItem() that allows a distributor to ship coffee", async() => {
    ...
    // 11th Test
    it("Testing smart contract function transferOwnership() that allows current owner transfer the onwership of contract", async() => {
        const supplyChain = await SupplyChain.deployed();

        // Transfer the ownership of contract to other account
        let tx = await supplyChain.transferOwnership(newContractOwner, {from: ownerID});
        
        // Verify the new contract owner
        let isNewOwner = await supplyChain.isOwner.call({from: newContractOwner});
        let isOldOwner = await supplyChain.isOwner.call({from: ownerID});
        assert.equal(true, isNewOwner);
        assert.equal(false, isOldOwner);
